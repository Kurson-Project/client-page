...
export function TypingEffect({
  ...
  React.useEffect(() => {
      ...
      while (isMounted) {
        ...
        for (let i = 0; i < currentText.length; i++) {
          ...
          await controls.start((j) =>
            j === i ? { opacity: 1, transition: { duration: 0.1 } } : {}
          )
          await sleep(50)
        }
        ...
        for (let i = currentText.length - 1; i >= 0; i--) {
          if (!isMounted) return
          await controls.start((j) =>
            j === i ? { opacity: 0, transition: { duration: 0.05 } } : {}
          )
          await sleep(50)
        }
    ...
    return () => {
      isMounted = false // stop loop saat unmount
    }
  }, [textIndex, texts, controls])
...
export function NeonText({ text, className }: { text: string, className?: string }) {
  const neonColors = ["#00ffff", "#ff00ff", "#ffff00", "#00ff00", "#ff6600", "#ff0066"];
    ...
    <h1>
      {text.split("").map((char, index) => (
        <motion.span
          ...
          style={{
            textShadow: `
              0 0 10px ${neonColors[index % neonColors.length]}
            `,
          }}
          animate={{ y: [0, -10, 0] }}
          transition={{
